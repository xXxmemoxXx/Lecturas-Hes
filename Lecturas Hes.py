...
# 2. CSS PARA DISEÑO DE FILTROS (CAJAS NEGRAS CON BORDE BLANCO SEGÚN TU IMAGEN)
st.markdown("""
    <style>
        .stApp { background-color: #000000 !important; color: white; }
        section[data-testid="stSidebar"] { background-color: #000000 !important; }
        
        /* Ocultar etiquetas externas de Streamlit */
        [data-testid="stWidgetLabel"] { display: none !important; }

        /* Diseño de caja sólida con borde blanco nítido */
        ...
        /* Texto dentro de la caja (Blanco y resaltado) */
...
with st.sidebar:
    ...
    if len(fecha_rango) == 2:
        # Se cargan todos los campos de la tabla HES
        df_hes = pd.read_sql(f"SELECT * FROM HES WHERE Fecha BETWEEN '{fecha_rango[0]}' AND '{fecha_rango[1]}'", mysql_engine)
        
        # --- FILTROS CON EL NOMBRE DENTRO DE LA CAJA ---
        ...
        for col in campos_filtros:
            if col in df_hes.columns:
                ...
                # El placeholder pone el nombre del campo DENTRO de la caja blanca
        ...
        if not df_hes.empty:
            ...
            for _, row in top10.iterrows():
                ...
                c2.markdown(f'<div style="width:{pct}%; background:#FF0000; height:6px; border-radius:2px; margin-top:5px;"></div>', unsafe_allow_html=True)
    else: 
        st.stop()

# 5. PROCESAMIENTO PARA MAPA (Manteniendo todos los campos para el popup detallado)
...
# 6. MAPA Y POPUP GIGANTE ORIGINAL
st.title("Medidores inteligentes - Tablero de consumos")

lat_centro = df_mapa['Latitud'].mean() if not df_mapa.empty else 21.8853
lon_centro = df_mapa['Longitud'].mean() if not df_mapa.empty else -102.2916
...
# Marcadores con el POPUP GIGANTE ORIGINAL RESTAURADO
for _, r in df_mapa.iterrows():
    if pd.notnull(r['Latitud']) and pd.notnull(r['Longitud']):
        ...
        <div style='font-family: Arial, sans-serif; font-size: 12px; width: 300px; color: #333; line-height: 1.4;'>
            ...
            <b>ClienteID_API:</b> {r.get('ClienteID_API', 'N/A')} - <b>Serie:</b> {r['Medidor']}<br>
        ...
        folium.CircleMarker(
            ...
            popup=folium.Popup(pop_html, max_width=350)).add_to(m)
