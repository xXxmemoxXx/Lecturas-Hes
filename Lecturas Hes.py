...
# ESTILO CSS PARA FILTROS TIPO CAJA SÓLIDA (MANTENIENDO TU IDENTIDAD VISUAL)
st.markdown("""
    <style>
        .stApp { background-color: #000000 !important; color: white; }
        section[data-testid="stSidebar"] { background-color: #000000 !important; }
        
        /* DISEÑO DE CAJA SÓLIDA PARA FILTROS */
        ...
        /* Etiquetas de los filtros arriba de la caja */
...
def get_mysql_engine():
    try:
        ...
        st.error(f"Error motor MySQL: {e}"); return None
...
with st.sidebar:
    ...
    opcion_rango = st.selectbox("Rango", ["Este mes", "Última semana", "Mes pasado", "Últimos 6 meses", "Este año", "Año pasado", "Personalizado"], index=0, label_visibility="collapsed")
    ...
    if len(fecha_rango) == 2:
        ...
        # --- FILTROS SIN COLUMNAS (DISEÑO SOLICITADO) ---
        ...
        for col in filtros_sidebar:
            if col in df_hes.columns:
                ...
                st.markdown(f"<p>{col}</p>", unsafe_allow_html=True)
                seleccion = st.multiselect(col, options=opciones, key=f"f_{col}", label_visibility="collapsed")
...
with col_map:
    ...
    for _, r in df_mapa.iterrows():
        if pd.notnull(r['Latitud']) and pd.notnull(r['Longitud']):
            ...
            folium.CircleMarker(location=[r['Latitud'], r['Longitud']], radius=2, color=color_hex, fill=True, fill_opacity=0.9, popup=folium.Popup(pop_html, max_width=350)).add_to(m)
